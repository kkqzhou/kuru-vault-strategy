...
def get_vault_strategy_state(start_date, end_date, market='MONUSDC'):
    ...
    df = client.query(
        f"""SELECT
            ...
            dislocation_bps
        FROM strategy_state where time >= '{start_str}'::timestamp and time < '{end_str}'::timestamp and market = '{market}'
        ORDER BY time
        """
    ).to_pandas()
...
def save_data(date, market):
    trades_df = get_vault_trades(date, date + pd.Timedelta(days=1), market=market)
    trades_output = os.path.join(OUTPUT_DIR, f'{market}_vault_trades_{date.strftime('%Y%m%d')}.csv')
    trades_df.to_csv(trades_output)
    print(f"Saved vault trades to {trades_output}")

    df = get_vault_strategy_state(date, date + pd.Timedelta(days=1), market=market)
    ...
    strategy_state_output = os.path.join(OUTPUT_DIR, f'{market}_vault_strategy_state_{date.strftime('%Y%m%d')}.csv')
